...
def save_shill(update, context):
    ...
    if len(context.args) > 0:
        ...
        if is_admin(update) or user_id == target_user_id:
            ...
            c.execute('INSERT INTO shills (shill, user_id, chat_id) VALUES (?, ?, ?)',
                      (shill, target_user_id, chat_id))
            conn.commit()
...
def get_user_shills(update, context):
    ...
    c.execute('SELECT shill FROM shills WHERE user_id=? AND chat_id=?',
...
updater = Updater(
    '6002292363:AAHftRSdNeXZ-BB4KAtvGwagRGlpR1n7JaU', use_context=True)
...
updater.dispatcher.add_handler(MessageHandler(
    Filters.text & ~Filters.command, save_shill))
